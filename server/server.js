...
import serviceAccountKey from "./blog-website-20629-firebase-adminsdk-w1h5a-8c16a06be9.json" assert { type: "json" };
  ...
  if (isAdmin) {
    ...
    if (!draft) {
      ...
      if (!tags.length || tags.length > 15) {
        return res.status(403).json({
          error: "Provide tags in order to publish the blog, Maximum 15",
        });
      }
    }
...
server.post("/add-comment", verifyJWT, (req, res) => {
  ...
  // console.log("replaying to", replying_to);
...
server.post("/delete-blog", verifyJWT, (req, res) => {
  ...
  if (isAdmin) {
    ...
    return res
      .status(500)
      .json({ error: "You have not permission to delete the blog" });
  }
});
